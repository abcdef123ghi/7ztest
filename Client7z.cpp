...
public:
  ...
  STDMETHOD(CryptoGetTextPassword)(BSTR *password, string & errmsg);
...
STDMETHODIMP CArchiveOpenCallback::CryptoGetTextPassword(BSTR *password, string & errmsg)
{
  ...
  //  PrintError("Password is not defined");
	  errmsg = "Password is not defined";
    return E_ABORT;
  }
...
public:
  ...
  STDMETHOD(GetStream)(UInt32 index, ISequentialOutStream **outStream, Int32 askExtractMode, string & errmsg);
  STDMETHOD(PrepareOperation)(Int32 askExtractMode, string & errmsg);
  STDMETHOD(SetOperationResult)(Int32 resultEOperationResult, string & errmsg);

  // ICryptoGetTextPassword
  STDMETHOD(CryptoGetTextPassword)(BSTR *aPassword, string & errmsg);
...
STDMETHODIMP CArchiveExtractCallback::GetStream(UInt32 index,
    ISequentialOutStream **outStream, Int32 askExtractMode,string & errmsg)
{
  *outStream = 0;
  _outFileStream.Release();

  {
	  const wchar_t * const kEmptyFileAlias = L"[Content]";
    // Get Name
    NCOM::CPropVariant prop;
  ...
  else
  {
    ...
    if (fi.Find(fullProcessedPath))
    {
      ...
      //  PrintError("Can not delete output file", fullProcessedPath);
		  errmsg = "Can not delete output file";
    ...
    if (!_outFileStreamSpec->Open(fullProcessedPath, CREATE_ALWAYS))
    {
     // PrintError("Can not open output file", fullProcessedPath);
		errmsg = "Can not open output file";
      return E_ABORT;
    }
...
STDMETHODIMP CArchiveExtractCallback::PrepareOperation(Int32 askExtractMode, string &errmsg)
{
	  const char * const kTestingString = "Testing     ";
	 const char * const kExtractingString = "Extracting  ";
	  const char * const kSkippingString = "Skipping    ";
  _extractMode = false;
...
STDMETHODIMP CArchiveExtractCallback::SetOperationResult(Int32 operationResult,string &errmsg)
{

	  const char * const kUnsupportedMethod = "Unsupported Method";
	  const char * const kCRCFailed = "CRC Failed";
	 const char * const kDataError = "Data Error";
	  const char * const kUnavailableData = "Unavailable data";
	  const char * const kUnexpectedEnd = "Unexpected end of data";
	  const char * const kDataAfterEnd = "There are some data after the end of the payload data";
	  const char * const kIsNotArc = "Is not archive";
	 const char * const kHeadersError = "Headers Error";
...
STDMETHODIMP CArchiveExtractCallback::CryptoGetTextPassword(BSTR *password ,string & errmsg)
{
  if (!PasswordIsDefined)
  {
    // You can ask real password here from user
...
public:
  ...
  STDMETHOD(GetStream)(UInt32 index, ISequentialInStream **inStream, string & errmsg);
  STDMETHOD(SetOperationResult)(Int32 operationResult);
  STDMETHOD(GetVolumeSize)(UInt32 index, UInt64 *size);
  STDMETHOD(GetVolumeStream)(UInt32 index, ISequentialOutStream **volumeStream);

  STDMETHOD(CryptoGetTextPassword2)(Int32 *passwordIsDefined, BSTR *password,  string & errmsg);
...
STDMETHODIMP CArchiveUpdateCallback::GetStream(UInt32 index, ISequentialInStream **inStream, string & errmsg)
{
  RINOK(Finilize());
...

  {
    ...
    if (!inStreamSpec->Open(path))
    {
     ...
     //   PrintError("WARNING: can't open file");
		  errmsg = "WARNING: can't open file";
...
STDMETHODIMP CArchiveUpdateCallback::CryptoGetTextPassword2(Int32 *passwordIsDefined, BSTR *password,string &errmsg)
{
  if (!PasswordIsDefined)
  {
    if (AskPassword)
...
int MY_CDECL main(int numArgs, const char *args[])
{
	 ...
	 const char * const kCopyrightString =
		"\n"
		"7-Zip"
		" (" kDllName " client)"
		" " MY_VERSION
	 ...
	 const char * const kIncorrectCommand = "incorrect command";
